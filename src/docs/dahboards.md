...
async function loadDashboard() {
  ...
  (async () => {
    ...
    function renderBanner({ charId, charName, birthday, gender, secStatus, corpId, corpName,
                             allianceId, allianceName, homeStationName, homeSolarSystemName = null, homeSystemSec,
            ...
            <div class="banner-stats-col">
              ...
              <div class="banner-stat-row">
                ...
                <span class="banner-stat-value banner-home-value">
                  ${homeSolarSystemName
                    ? `<span style="color:${sysMeta.color};">${escHtml(homeSolarSystemName)}</span><span style="color:var(--text-3);margin:0 3px;">|</span><span>${escHtml(homeStationName || '—')}</span>`
    ...
    try {
      ...
      // ── Home location — resolved via locator (clones endpoint) ──────────
      // The character sheet and location table don't carry the home station ID.
      // The real source is clones.home_location.location_id (NPC station or
      // Upwell structure). We call resolveLocation() so the locator's full
      // fallback chain (local DB → ESI → Hammertime → zKillboard) is used,
      // exactly the same path as assets and jump clones.
      let homeStationName     = loc?.station_name || loc?.solar_system_name || '—';
      let homeSolarSystemName = null;
      let homeSystemSec       = null;
      try {
        const clones = await window.eveAPI.getClones(mainAccount.characterId);
        const homeLocId = clones?.home_location?.location_id;
        if (homeLocId) {
          const resolved = await window.eveAPI.resolveLocation(homeLocId, mainAccount.characterId);
          if (resolved?.name && !resolved.name.startsWith('Location ')) {
            homeStationName = resolved.name;
          }
          if (resolved?.solar_system_name) {
            homeSolarSystemName = resolved.solar_system_name;
          }
          if (typeof resolved?.security_status === 'number') {
            homeSystemSec = resolved.security_status;
          }
          logToConsole(`Home station resolved: ${homeStationName} (${homeLocId})`, 'info');
        }
      } catch (homeErr) {
        logToConsole(`Home station resolve failed: ${homeErr.message}`, 'info');
        // homeStationName already has the loc fallback set above — keep it
      }
      ...
      renderBanner({
        ...
        homeStationName, homeSolarSystemName, homeSystemSec,
        bloodlineName,
        implants,
